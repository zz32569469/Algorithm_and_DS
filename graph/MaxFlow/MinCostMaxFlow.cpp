...
typedef int Tcost;
  ...
  int V, s, t;
  vector<Edge> g[MAXV];
  void init(int n, int _s, int _t){//點的數量，源點，匯點
    V = n; s = _s; t = _t;
  ...
  }//加入一條點 u 連到點 v 流量限制為 cap 花費為 w 的邊 
  Tcost d[MAXV];
  ...
  pair<int,Tcost> solve(){
    ...
    return {mxf,mnc};//回傳<最大流量，最小花費>
} }flow;//MinCostMaxFlow  O(V^2·E^2)
